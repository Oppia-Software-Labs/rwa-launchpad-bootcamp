...
function requireContractId(): string {
  if (!config.contractId) {
    throw new Error(
      "Contract not deployed yet — set NEXT_PUBLIC_CONTRACT_ID in your environment.",
    );
  }
...
/**
 * Read AssetInfo from contract instance storage (no public getter in the contract).
 */
...
export async function balance(
  ...
  const result = await simulateRead<bigint | number | string>(
    "balance",
    [addressScVal(id)],
    sourceAddress,
  );
...
export async function mint(
  ...
  const { hash } = await invoke(
    "mint",
    [addressScVal(admin), addressScVal(to), i128ScVal(amount)],
    admin,
    signTransaction,
  );
  return { hash, result: undefined };
...
export async function transfer(
  ...
  const { hash } = await invoke(
    "transfer",
    [addressScVal(from), addressScVal(to), i128ScVal(amount)],
    from,
    signTransaction,
  );
  return { hash, result: undefined };
...
export async function setWhitelist(
  ...
  const { hash } = await invoke(
    "set_whitelist",
    [addressScVal(admin), addressScVal(investor), boolScVal(approved)],
    admin,
    signTransaction,
  );
  return { hash, result: undefined };
...
export async function invest(
  ...
  const { hash, retval } = await invoke(
    "invest",
    [addressScVal(investor), i128ScVal(paymentAmount)],
    investor,
    signTransaction,
  );
  const minted = retval
    ? BigInt(scValToNative(retval) as string | number | bigint)
    : BigInt(0);
  return { hash, result: minted };
}
