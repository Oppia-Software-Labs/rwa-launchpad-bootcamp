    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="max-w-2xl text-body-sm text-text-secondary">
          Admin-gated calls for the Día 3 launchpad. Connected as{" "}
      ...
      <ContractGate>
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card title="Withdraw" metadata="withdraw(admin, to, amount)">
            <form
              className="space-y-4"
              onSubmit={(e: FormEvent) => {
                e.preventDefault();
                void runAction(setWithdrawState, () =>
                  withdraw(
                    address,
                    withdrawTo.trim(),
                    withdrawAmount,
            ...
            >
              <Field label="To" hint="Receives payment tokens held by the contract.">
                <input
                  className={inputClassName}
                  required
                  value={withdrawTo}
              ...
              {withdrawState.hash ? (
                <TxSuccess hash={withdrawState.hash} />
              ) : null}
              <Button
                type="submit"
                variant="secondary"
          ...
          >
            <div className="flex flex-wrap items-center gap-3">
              <Button
                ...
                disabled={!networkOk}
                onClick={() =>
                  void runAction(setPauseState, () =>
                    pause(address, signTransaction),
                  )
                }
              >
                Pause
              ...
              <Button
                ...
                onClick={() =>
                  void runAction(setPauseState, () =>
                    unpause(address, signTransaction),
                  )
                }
              >
                Unpause
              </Button>
              {pauseState.error ? <FormError message={pauseState.error} /> : null}
              {pauseState.hash ? <TxSuccess hash={pauseState.hash} /> : null}
            </div>
          </Card>
        </div>
      </ContractGate>
